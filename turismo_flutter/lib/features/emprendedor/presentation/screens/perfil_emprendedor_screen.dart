...
import 'package:turismo_flutter/features/emprendedor/presentation/screens/formulario_actualizar_emprendedor.dart';

class PerfilEmprendedorScreen extends StatefulWidget {
  const PerfilEmprendedorScreen({super.key});

  @override
  State<PerfilEmprendedorScreen> createState() => _PerfilEmprendedorScreenState();
}

class _PerfilEmprendedorScreenState extends State<PerfilEmprendedorScreen> {
  @override
  void initState() {
    super.initState();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: BlocBuilder<UsuarioEmprendedorBloc, UsuarioEmprendedorState>(
        buildWhen: (previous, current) =>
        current is UsuarioEmprendedorLoading ||
            current is UsuarioEmprendedorProfileLoaded ||
            current is UsuarioEmprendedorError,
        builder: (context, state) {
          if (state is UsuarioEmprendedorLoading) {
            return const Center(child: CircularProgressIndicator());
          } else if (state is UsuarioEmprendedorProfileLoaded) {
            final UsuarioEmprendedorResponse usuario = state.usuario;
                      ...
                      onPressed: () async {
                        final result = await showModalBottomSheet<bool>(
                          ...
                          builder: (context) => Padding(
                            ...
                            child: FormularioActualizarEmprendedor(usuario: usuario),
                          ),
                        );
                      }
                  ),
          ...
          } else if (state is UsuarioEmprendedorError) {

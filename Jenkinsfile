    ...
    steps {
        timeout(time: 4, unit: 'MINUTES') {
            withCredentials([string(credentialsId: 'Sonarqube', variable: 'SONAR_TOKEN')]) {
                sh "${tool 'MAVEN_HOME'}/bin/mvn org.sonarsource.scanner.maven:sonar-maven-plugin:3.9.0.2155:sonar -Pcoverage -Dsonar.login=${SONAR_TOKEN} -f turismo-spring-boot/pom.xml"
            }
        }
    }
}
